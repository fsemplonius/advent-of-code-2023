...
function cmpCards($card1, $card2) {
  $strength = array ('A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J');
  ...
  foreach ($cmpCards as $cards) {
    ...
    $fCard[$n] = $cardsStr;
    $cnt1Cards = array_count_values($cardsStr);
    if (($nJ=$cnt1Cards[12]) > 0) {	// Jokers
      unset($cnt1Cards[12]);
      rsort($cnt1Cards);
      $cnt1Cards[array_key_first($cnt1Cards)] += $nJ;
    }
    $cntCards = array_count_values($cnt1Cards);
    if ($cntCards[5] == 1)
